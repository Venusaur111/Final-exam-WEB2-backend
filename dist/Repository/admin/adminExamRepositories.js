...
export class ExamRepository {
    ...
    async findByCourse(courseId) {
        const result = await pool.query(`SELECT id, course_id AS "courseId", title, description,
             ...
             FROM exams
             WHERE course_id = $1
             ORDER BY start_at DESC`, [courseId]);
        return result.rows;
    }
    async findById(id) {
        const result = await pool.query(`SELECT e.id, e.course_id AS "courseId", e.title, e.description,
    ...
    async update(id, data) {
        const result = await pool.query(`UPDATE exams
    ...
    async hasAttempts(id) {
        const result = await pool.query(`SELECT 1 FROM attempts WHERE exam_id = $1 LIMIT 1`, [id]);
        return (result.rowCount ?? 0) > 0;
    }
    async isWithinWindow(id, now = new Date()) {
        const result = await pool.query(`SELECT 1 FROM exams WHERE id = $1 AND start_at <= $2 AND end_at >= $2`, [id, now]);
        return (result.rowCount ?? 0) > 0;
    }
    async getResultsSummary(examId) {
        const maxScoreResult = await pool.query(`SELECT COALESCE(SUM(points), 0)::int AS "maxScore" FROM questions WHERE exam_id = $1`, [examId]);
        const maxScore = maxScoreResult.rows[0]?.maxScore ?? 0;
        const result = await pool.query(`SELECT u.id AS "studentId", u.name AS "studentName", u.email AS "studentEmail",
                    a.score, a.submitted_at AS "submittedAt", $2::int AS "maxScore"
             FROM attempts a
             JOIN users u ON u.id = a.student_id
             WHERE a.exam_id = $1
             ORDER BY a.submitted_at DESC`, [examId, maxScore]);
        const rows = result.rows;
        const attemptsCount = rows.length;
        const average = attemptsCount === 0
            ? 0
