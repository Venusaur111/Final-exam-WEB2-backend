...
export class StudentExamRepository {
        ...
        try {
            ...
            for (const answer of answers) {
                await client.query(`INSERT INTO answers (attempt_id, question_id, choice_id)
                     VALUES ($1, $2, $3)`, [attempt.id, answer.questionId, answer.choiceId ?? null]);
            }
            const correction = await this.buildCorrection(client, attempt.id, examId);
            await client.query(`UPDATE attempts SET score = $2 WHERE id = $1`, [attempt.id, correction.score]);
    ...
    async findMyResults(studentId) {
        const attempts = await pool.query(`SELECT a.id AS "attemptId", e.id AS "examId", e.title AS "examTitle",
                    c.name AS "courseName", a.score, a.submitted_at AS "submittedAt"
             FROM attempts a
        ...
        for (const row of attempts.rows) {
            const correction = await this.buildCorrection(pool, row.attemptId, row.examId);
            results.push({
                ...correction,
    ...
    async getCorrectionForAttempt(attemptId, studentId) {
        const attemptResult = await pool.query(`SELECT a.id, a.exam_id AS "examId", a.score, e.title AS "examTitle", c.name AS "courseName",
        ...
        if (!attempt)
            return null;
        const correction = await this.buildCorrection(pool, attemptId, attempt.examId);
        return {
            ...correction,
    ...
    async buildCorrection(queryable, attemptId, examId) {
        const correctionResult = await queryable.query(`SELECT q.id AS "questionId", q.statement, q.points,
                    ...
                    (a.choice_id IS NOT NULL AND a.choice_id = correct.id) AS "isCorrect"
             FROM questions q
             LEFT JOIN answers a
        ...
        return { attemptId, examId, score, maxScore, corrections };
    }
}
