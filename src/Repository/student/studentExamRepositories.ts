...
export class StudentExamRepository {
            ...
            for (const answer of answers) {
                await client.query(
                    `INSERT INTO answers (attempt_id, question_id, choice_id)
                     VALUES ($1, $2, $3)`,
                    [attempt.id, answer.questionId, answer.choiceId ?? null]
                );
            }
    ...
    async findMyResults(studentId: string): Promise<ExamCorrection[]> {
    ...
    async getCorrectionForAttempt(attemptId: string, studentId: string): Promise<ExamCorrection | null> {
        const attemptResult = await pool.query(
            `SELECT a.id, a.exam_id AS "examId", a.score, e.title AS "examTitle", c.name AS "courseName",
    ...
    private async buildCorrection(
        ...
        const correctionResult = await queryable.query(
