...
import { PublicUser, User, CreateStudentInput, UpdateStudentInput } from "../../models/userModel.js";
...
export class UserService {
    ...
    async getStudentById(id: string): Promise<PublicUser> {
    ...
    async getAllStudents(): Promise<PublicUser[]> {
        const students = await this.userRepo.findAllStudents();
        return students.map(toPublicUser);
    }

    async createStudent(data: Omit<CreateStudentInput, "passwordHash"> & { password: string; name?: string }): Promise<PublicUser> {
    ...
    async updateStudent(
        id: string,
        data: UpdateStudentInput & { password?: string }
    ): Promise<PublicUser> {
